...
- 工作空间 MVP：新增“空间”Tab，使用 `localStorage` 保存工作空间列表并内置一个示例空间；可填写空间名称、项目目标并选择小黑/小橙/小金作为成员。空间详情展示名称、目标、成员、创建时间，以及从现有 `tasks` 按成员过滤得到的最近 5 条真实任务，不生成空间任务占位数据。
- 空间内专家团：在当前空间输入问题后，仅向该空间成员复用现有消息发送链路；投递消息包含空间名称、目标、成员角色视角和用户问题。页面只展示每位成员的 Hermes 已发送、兜底已入队或投递失败状态，完成后刷新 outbox 与任务历史，不伪造回答。
