...
function App() {
  ...
  const tabs = [
    ...
    { id: 'tasks', label: '任务动态' },
    { id: 'workflow', label: '工作流 v4' },
  ];

  return (
    <div className="app" style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div className="tab-bar" style={{ display: 'flex', borderBottom: '1px solid #e8e6e1', background: '#f8f6f3', padding: '0 8px' }}>
        {tabs.map(tab => (
          <button
            key={tab.id}
            ...
            style={{
              ...
              borderBottom: activeTab === tab.id ? '4px solid #477fac' : '4px solid transparent',
      ...
      <div className="main-content" style={{ flex: 1, overflow: 'hidden' }}>
        ...
        {activeTab !== 'workflow' && (
          <div style={{ padding: '120px 60px', textAlign: 'center', color: '#999', fontSize: '16px' }}>
            其他 Tab 保持原有功能<br/>当前专注工作流 v4 优化版（微信 Gateway 已支持）
          </div>
        )}
      </div>
    </div>
